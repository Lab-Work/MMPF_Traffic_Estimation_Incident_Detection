...
def left_boundary_statistic(laneNumber, Vmax, inflow):
    lbDensity = (inflow+ random.normal(0,100))/Vmax
    lbModel = laneNumber
...
## Load Measurements and true
    ...
    directorySave = '/Users/Ren/Dropbox/SourceCode/test/EnKF/Result_inflow/'
...
#    print 'model error is', sum(abs(modelError))
    ...
    save( directorySave + str(PR)+'lag0'+'estDen'+str(inflow),estimatedState)
    save( directorySave + str(PR)+'lag0'+'estModel'+str(inflow),estimatedModel)

    plot_density(densityTrue[0:180], savefigure, 'trueDensityPlot'+str(inflow), directorySave)
    plot_density(estimatedState, savefigure, str(PR)+'lag0'+'estDen'+str(inflow), directorySave)
    plot_parameter(estimatedModel, savefigure, str(PR)+'lag0'+'EstModel'+str(inflow), directorySave)
