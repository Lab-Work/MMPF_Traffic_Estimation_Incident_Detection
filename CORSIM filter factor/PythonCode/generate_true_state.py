...
def collect_true_density_speed(vehTrajectory, dataTime, dt, dx, cellNumber, simulationTime):
## get the true density and speed for all cells at all time steps, the data are collected at the beginning at each time step dt
        timeStep = int(simulationTime/(dt))
        cellCount = zeros((timeStep, cellNumber))
                        ...
                        try:
                                for j in range(len(dataTime[timeID][:,0])):
                                        ...
                                        if locationID < 11.0: ## vehicle outside the domain
                                                cellCount[timeIndex, locationID] = cellCount[timeIndex, locationID]+1
...
def plot(cellDensity, cellSpeed, saveDirectory, fileName):
        ...
        plt.colorbar()
        plt.savefig(saveDirectory+'FigTrueDensity'+fileName+'.pdf', bbox_inches='tight')
#        plt.show()
        plt.clf()
        ...
        plt.xlabel('Cell Number',fontsize=30)
        plt.colorbar()
        plt.savefig(saveDirectory+'FigTrueSpeed'+fileName+'.pdf', bbox_inches='tight')
#        plt.show()
        plt.clf()
...
#        fileList = ['L3B1F6000S65R50']
        ...
        for fileName in fileList:
                ...
                cellDensity, cellSpeed = collect_true_density_speed(vehTrajectory, dataTime, dt, dx, cellNumber, simulationTime)
                plot(cellDensity, cellSpeed, saveDirectory, fileName)

                save(saveDirectory + 'TrueDensity' + fileName, cellDensity)
                save(saveDirectory + 'TrueSpeed' + fileName, cellSpeed)                
